...
const EXCLUDE = [/spotifyd/, /firefox/, /zen/, /brave/];
...
function focusEntry(entry: string) {
	...
	const client = hypr.clients.find((c) => {
		...
		return cls === target || cls.includes(target) || target.includes(cls);
	});
...
function Player({ player }: { player: Mpris.Player }) {
	...
	const text = createComputed(() => {
		...
		const t = title();
		const a = artist();
		return s === Mpris.PlaybackStatus.STOPPED
			? "Stopped"
			: `${t || ""} - ${a || ""}`;
	});
	...
	return (
		<overlay
			class="media-button-overlay"
			$={(self) => {
				for (const child of self.get_children()) {
					if (child !== self.child) {
						self.set_overlay_pass_through(child, true);
					}
				}
			}}
		>
...
export default function Media() {
	...
	return (
		<box spacing={8} visible={visible}>
			<For each={players(filter)}>{(player) => <Player player={player} />}</For>
		</box>
	);
}
