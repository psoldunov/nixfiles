  ...
  alwaysOn = {
    ...
    update_system = pkgs.writeShellScriptBin "update_system" ''
      ...
      if [ "$HOST" = "$LOCAL_HOST" ]; then
        ...
        REBUILD_CMD=(nixos-rebuild switch --flake ".#$HOST" --target-host "$TARGET_HOST" --build-host "$TARGET_HOST" --sudo --ask-sudo-password --show-trace --upgrade-all)
      fi
      if "''${REBUILD_CMD[@]}"; then
    ...
    rebuild_system = pkgs.writeShellScriptBin "rebuild_system" ''
      ...
      if [ "$HOST" = "$LOCAL_HOST" ]; then
        ...
        TARGET_HOST="psoldunov@''${HOST,,}"
        REBUILD_CMD=(nixos-rebuild switch --flake ".#$HOST" --target-host "$TARGET_HOST" --build-host "$TARGET_HOST" --sudo --ask-sudo-password --show-trace)
      fi
      if "''${REBUILD_CMD[@]}"; then
        if ! git diff --cached --quiet || ! git diff --quiet; then
